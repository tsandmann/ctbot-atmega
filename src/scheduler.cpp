...
Scheduler::Task::Task(const uint16_t id, const uint16_t period, task_func_t&& func, task_func_data_t&& func_data)
    : id_ { id }, period_ { static_cast<uint16_t>(Timer::ms_to_ticks(period)) }, next_runtime_ { 0 }, active_ { true }, func_ { std::move(func) }, func_data_ { std::move(func_data) } {}
...
Scheduler::Task::Task(const uint16_t id, const uint16_t period, const uint32_t next_run, task_func_t&& func, task_func_data_t&& func_data)
    : id_ { id }, period_ { static_cast<uint16_t>(Timer::ms_to_ticks(period)) }, next_runtime_ { next_run }, active_ { true }, func_ { std::move(func) }, func_data_ { std::move(func_data) } {}
...
void Scheduler::run() {
    while (running_ && (! task_queue_.empty())) {
        ...
        while (next_runtime > now) {
            do {
                _SLEEP_CONTROL_REG = (_SLEEP_CONTROL_REG & ~(BV_8(SM0) | BV_8(SM1) | BV_8(SM2))) | (SLEEP_MODE_IDLE);
            } while (0);
            sleep_enable();
            sleep_cpu();
...
uint16_t Scheduler::task_add(const std::string& name, const uint16_t period, task_func_t&& func, task_func_data_t&& func_data) {
    ...
    std::cout << F("Scheduler::task_add(): task \"") << task_names_[next_id_] << F("\" with id 0x") << std::hex << task.id_ << F(" and period of ")
        << std::dec << Timer::ticks_to_us(task.period_) << F(" us added.\n");
    return next_id_++;
}
